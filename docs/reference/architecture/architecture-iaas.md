# Architecture IaaS - VM Scale Set

## Vue d'ensemble

L'approche IaaS (Infrastructure as a Service) utilise des machines virtuelles dans un VM Scale Set avec Public IPs automatiquement assignées. Cette architecture offre un contrôle total sur l'infrastructure et une flexibilité maximale, au prix d'une complexité accrue et d'une maintenance manuelle.

> **Note**: Cette architecture n'utilise pas de Load Balancer en raison de restrictions Azure Policy dans la subscription Epitech. Les instances VMSS reçoivent automatiquement des adresses IP publiques.
...
graph TB
    ...
    subgraph "Azure - Resource Group: rg-nan_1"
        subgraph "Infrastructure IaaS"
            subgraph "VMSS avec Public IPs"
                VMSS[VM Scale Set<br/>tc-dev-vmss-frc-01]
                VM1[VM Instance 0<br/>Ubuntu 22.04 + Docker<br/>Public IP auto]
                VM2[VM Instance 1<br/>Ubuntu 22.04 + Docker<br/>Public IP auto]
            end
            
            subgraph "Auto-scaling"
                ASProfile[Profil: 1-2 instances dev, 2-5 prod]
    ...
    Users -->|HTTP Port 80| VM1
    Users -->|HTTP Port 80| VM2
    ...
    class VMSS,VM1,VM2 iaasService
```

## Composants IaaS

### Public IPs (Auto-assignées aux instances VMSS)

#### Caractéristiques

- **Type**: Dynamiques, automatiquement assignées à chaque instance VMSS
- **SKU**: Standard
- **Configuration**: Configurée au niveau du VMSS, pas de ressource dédiée

#### Usage

Chaque instance du VMSS reçoit automatiquement une adresse IP publique. Pour obtenir les IPs :

```bash
# Lister les IPs publiques des instances VMSS
az vmss list-instance-public-ips \
  --resource-group rg-nan_1 \
  --name tc-dev-vmss-frc-01

# Obtenir l'IP d'une instance spécifique
az vmss list-instance-public-ips \
  --resource-group rg-nan_1 \
  --name tc-dev-vmss-frc-01 \
...
- **Instances**: 
  - **Dev**: 1 minimum, 2 maximum
  - **Prod**: 2 minimum, 5 maximum
- **Public IPs**: Automatiquement assignées à chaque instance
...
flowchart TD
    ...
    TF1 --> TF2[Terraform: VMSS avec Public IPs]
    TF2 --> Wait[Attente cloud-init<br/>2-3 minutes]
...
Crée:
- VM Scale Set avec Public IPs automatiques
- 1-2 instances Ubuntu selon l'environnement
...
sequenceDiagram
    ...
    participant VM1 as VM Instance 1 (Public IP)
    participant VM2 as VM Instance 2 (Public IP)
    participant DB as MySQL
    
    Note over VM1,VM2: Chaque instance a<br/>sa propre Public IP
    
    U->>VM1: HTTP GET /api/users
    VM1->>VM1: Container traite
    VM1->>DB: SELECT * FROM users
    DB-->>VM1: Résultats
    VM1-->>U: HTTP 200 + JSON
    
    U->>VM2: HTTP GET /api/products
    VM2->>VM2: Container traite
    VM2->>DB: SELECT * FROM products
    DB-->>VM2: Résultats
    VM2-->>U: HTTP 200 + JSON
    
    Note over U,VM2: L'utilisateur peut accéder<br/>directement à chaque instance
```
...
- Accès SSH direct via Public IP de chaque instance
- Authentification par clé SSH uniquement (pas de mot de passe)

```bash
# Obtenir les Public IPs des instances
az vmss list-instance-public-ips \
  --resource-group rg-nan_1 \
  --name tc-dev-vmss-frc-01

# SSH vers instance spécifique
ssh -i ~/.ssh/terracloud-key azureuser@<INSTANCE_PUBLIC_IP>
```
...
## Limitations Azure Policy

### Restriction Load Balancer

Dans l'environnement Epitech, une Azure Policy bloque la création de Load Balancers :

```
Policy: deny loadbalancer
Policy Definition ID: f82b9589-d7ea-41d4-965c-135b9737b72c
```

#### Impact

- **Architecture simplifiée** : Public IPs directement sur instances VMSS
- **Pas de distribution de charge automatique** : Chaque instance a sa propre IP
- **Accès direct aux instances** : Connexion directe sur IP publique de l'instance

#### Solution de contournement

L'architecture actuelle assigne automatiquement des IPs publiques aux instances VMSS. Pour la répartition de charge dans un contexte production réel, considérez :

1. **Application Gateway** (Layer 7) - Peut être autorisé par la policy
2. **Azure Front Door** - Service global de distribution
3. **Traffic Manager** - DNS-based load balancing
...
| Public IPs | 2 IPs dynamiques | ~6 |
| Bande passante | ~10 GB sortant | ~1 |
| **Total IaaS** | | **~67** |
| Infrastructure partagée | (ACR + MySQL) | ~19 |
| **Total global** | | **~86** |

**Note** : Sans Load Balancer, économie de ~20 EUR/mois par rapport à l'architecture complète.
