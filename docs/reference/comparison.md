...
| Aspect | PaaS | IaaS |
|--------|------|------|
| **Composants principaux** | App Service Plan + Web App | VMSS avec Public IPs |
| **Nombre de ressources Azure** | 6 | 10+ |
| **Isolation réseau** | Optionnelle (VNet Integration) | Native (subnet dédié) |
| **Load balancing** | Automatique (multi-instances) | Aucun (IPs publiques directes) |
| **Accès direct aux serveurs** | Non (SSH limité au conteneur) | Oui (SSH via IP publique) |
...
```mermaid
pie title Répartition des coûts IaaS (~53€/mois - Dev)
    "VMSS (1x B2s)" : 30
    "MySQL B1ms" : 15
    "Container Registry" : 4
    "Public IPs" : 3
...
> **Note** : Architecture IaaS simplifiée sans Load Balancer en raison de restrictions Azure Policy.

#### Comparaison détaillée des coûts

| Ressource | PaaS | IaaS (Dev) | Différence |
|-----------|------|------|------------|
| **Compute** | App Service Plan B1: 12€ | 1x VM B2s: 30€ | +18€ |
| **Load Balancing** | Inclus | Non applicable: 0€ | 0€ |
| **IP Publique** | Inclus | IP Dynamique: 3€ | +3€ |
| **Base de données** | MySQL B1ms: 15€ | MySQL B1ms: 15€ | =0€ |
| **Registry** | ACR Basic: 4€ | ACR Basic: 4€ | =0€ |
| **Bande passante** | ~1€ | ~1€ | =0€ |
| **Total mensuel** | **~32€** | **~53€** | **+21€** |
